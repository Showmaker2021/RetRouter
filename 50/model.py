...
class DynamicTaskMemory(nn.Module):
    ...
    def forward(
        self,
        S_global: Tensor,    # (B, num_starts, H, D_q, D_q)
        task_emb: Tensor,    # (BS, 5, D) — already batchified flat
        q_base_flat: Tensor, # (BS, H*D)
        num_starts: int,
    ) -> Tensor:
        """
        All computation in flat BS = B*num_starts space.
        Returns delta_q: (BS, H*D_q) added to q_base in decoder.
        """
        B, S, H, D_q, _ = S_global.shape
        BS = B * S

        # --- S_global → K, V ---
        # mean-pool state matrix per head: (B,S,H,D_q,D_q) → (B,S,H,D_q)
        S_vec    = S_global.mean(dim=-1)                          # (B, S, H, D_q)
        S_vec_bs = S_vec.reshape(BS, 1, H * D_q)                 # (BS, 1, H*D_q)

        K = self.W_S_k(S_vec_bs).reshape(BS, 1, H, D_q).transpose(1, 2)  # (BS,H,1,D_q)
        V = self.W_S_v(S_vec_bs).reshape(BS, 1, H, D_q).transpose(1, 2)

        # --- task_emb → Q  (already flat BS) ---
        Q = self.W_task_q(task_emb).reshape(BS, 5, H, D_q).transpose(1, 2)  # (BS,H,5,D_q)

        # --- Cross-attention: task tokens attend to memory state ---
        score  = torch.matmul(Q, K.transpose(-2, -1)) * self.scale  # (BS,H,5,1)
        weight = torch.softmax(score, dim=-1)
        ctx    = torch.matmul(weight, V)                             # (BS,H,5,D_q)

        # Pool task tokens → project to embedding dim
        ctx      = ctx.mean(dim=2).reshape(BS, H * D_q)             # (BS, H*D_q)
        task_ctx = self.norm(self.W_task_out(ctx))                   # (BS, D_emb)
