...
class VRP_Encoder(nn.Module):
    ...
    def forward(self, td: TensorDict) -> Tensor:
        ...
        # node_feats for decay: [dl, db, tw_s, tw_e, st] — shape (B, N+1, 5)
        decay_feat = torch.cat([
            td["demand_linehaul"][..., :, None],    # (B, N+1, 1) — 0 for depot
            td["demand_backhaul"][..., :, None],
            td["time_windows"],                      # (B, N+1, 2)
            td["service_time"],                      # (B, N+1, 1)
        ], dim=-1)  # (B, N+1, 5)
