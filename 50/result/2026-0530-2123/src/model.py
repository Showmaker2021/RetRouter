...
class VRP_Decoder(nn.Module):
    ...
    def forward(
        ...
        """
        Returns:
            log_probs: (batch*num_starts, N+1)
            mask:      (batch*num_starts, N+1)
            S_local:   updated  (B, num_starts, H, D, D)
            S_global:  updated
        """
        td_unb = unbatchify(td, num_starts)  # (B, num_starts, ...)

        # Current node embedding
        cur_node_embed = gather_by_index(cache.node_embeddings, td_unb["current_node"])
        # (B, num_starts, embedding_dim)

        # State features
        rl = td_unb["vehicle_capacity"] - td_unb["used_capacity_linehaul"]
        rb = td_unb["vehicle_capacity"] - td_unb["used_capacity_backhaul"]
        state = torch.cat([rl, rb, td_unb["current_time"],
                           td_unb["current_route_length"], td_unb["open_route"]], -1)
        # (B, num_starts, 5)

        context = torch.cat([cur_node_embed, state], -1)  # (B, S, D+5)
        B, S_num, _ = context.shape
        H = self.model_params['head_num']
        D = self.model_params['qkv_dim']

        # --- Retention Q/K/V ---
        q_ret = reshape_by_heads(
            self.Wq_context(context), H
        ).transpose(1, 2)  # (B, S, H, D)  [note: reshape_by_heads gives (B,H,S,D)]
        # Fix: reshape manually to get (B, S, H, D)
        q_ret = self.Wq_context(context).reshape(B, S_num, H, D)  # (B, S, H, D)
        k_ret = self.Wk_ret(cur_node_embed).reshape(B, S_num, H, D)
        v_ret = self.Wv_ret(cur_node_embed).reshape(B, S_num, H, D)

        # Detect depot visits: current_node == 0
        at_depot = (td_unb["current_node"].squeeze(-1) == 0)  # (B, S)

        # Depot embedding from encoder output (node 0)
        depot_embed = cache.node_embeddings[:, :1, :]  # (B, 1, D_embed)

        # --- Hierarchical Route Memory ---
        memory_out, S_local, S_global = self.hrm(
            q_ret, k_ret, v_ret, S_local, S_global, at_depot, depot_embed
        )
        # memory_out: (B, S, embedding_dim)

        # --- Cross-attention: memory_out → encoder nodes ---
        # encoder keys/values are precomputed (cache.glimpse_key / glimpse_val)
        # glimpse_key: (B, H, N+1, D)  (precomputed by VRPModel)
        mask = td_unb["action_mask"]  # (B, S, N+1)

        q_cross = reshape_by_heads(self.Wq_cross(memory_out), H)  # (B, H, S, D)
        # cross-attention with mask
        cross_out = _multi_head_cross_attention(
            q_cross, cache.glimpse_key, cache.glimpse_val, mask
        )  # (B, S, H*D)
        mh_out = self.cross_combine(cross_out)  # (B, S, embedding_dim)
        mh_out = self.norm(mh_out + memory_out)

        # --- Pointer ---
        # logit_key = node_embeddings.T: (B, D, N+1)
        score = torch.matmul(mh_out, cache.logit_key)  # (B, S, N+1)
        score_scaled = score / self.model_params['sqrt_embedding_dim']

        # Rearrange to (B*S, N+1)
        logits = rearrange(score_scaled, "b s l -> (s b) l", s=S_num)
        mask_out = rearrange(mask, "b s l -> (s b) l", s=S_num)

        logits = torch.tanh(logits) * self.model_params['logit_clipping']
        logits[~mask_out] = float("-inf")

        return F.log_softmax(logits, dim=-1), mask_out, S_local, S_global


def _multi_head_cross_attention(q, k, v, mask=None):
    """
    Standard cross-attention for decoder → encoder.
    q: (B, H, S, D)
    k: (B, H, N, D)
    v: (B, H, N, D)
    mask: (B, S, N)  bool — True = valid
    Returns: (B, S, H*D)
    """
    B, H, S, D = q.shape
    N = k.size(2)
    scale = D ** -0.5
    score = torch.matmul(q, k.transpose(-2, -1)) * scale  # (B, H, S, N)
    if mask is not None:
        # mask: (B, S, N) → (B, 1, S, N)
        score = score + mask.unsqueeze(1).float().masked_fill(~mask.unsqueeze(1), float('-inf')).clamp(min=0) * 0
        # Proper masking: -inf where mask is False
...
class VRPModel(nn.Module):
    ...
    def forward(self, td, env):
        ...
        # Precompute cross-attention K/V from encoder (fixed)
        ...
        cache = PrecomputedCache(node_embed, decoder_k, decoder_v, decoder_logit_key)

        # Batchify node_embeddings for multi-start
        node_embed_ms = batchify(node_embed, num_starts)
        decoder_k_ms = batchify(decoder_k, num_starts)
        decoder_v_ms = batchify(decoder_v, num_starts)
        decoder_logit_key_ms = batchify(decoder_logit_key, num_starts)
        cache_ms = PrecomputedCache(node_embed_ms, decoder_k_ms, decoder_v_ms, decoder_logit_key_ms)

        # --- Initialize hierarchical memory ---
        # S_local / S_global shape: (B, num_starts, H, qkv_dim, qkv_dim)
        ...
        while not td["done"].all():
            logprobs, mask, S_local, S_global = self.decoder(
                td, cache_ms, num_starts, S_local, S_global
            )
            if self.training:
